...
| `IPDB_API_KEY` | *(empty)* | API key for [avatoris.com](https://avatoris.com) ([API docs](https://avatoris.com/api)), used to show a country flag per hop. Without it, flags are silently skipped — everything else still works. Lookups always go through avatoris.com (hardcoded, not self-hostable) — a free public instance provided alongside this project, not affiliated with the original IPDB project. |
